...
class GPT(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        # RoPE 的 max_len 应独立于 config.max_len（后者是训练截断长度）。
        # 2048 保证常用推理无需触发动态计算；即使超出也有 _get_rotations 兜底。
        self.layers = nn.ModuleList([
            DecoderLayer(config.d_model, config.n_heads, config.d_ff,
                         config.dropout, max_len=2048)
    ...
    def decode_step(
        ...
        返回: (logits, new_cache)
        """
        # 增量解码时新 token 在序列末尾，可看到所有已缓存 token，
        # 无需 causal mask。推理 prompt 不 padding，也无需 padding mask。
        h = self.embedding(token) * math.sqrt(self.d_model)

        new_cache: list = []
        for i, layer in enumerate(self.layers):
            h, kv = layer(h, mask=None, past_kv=past_cache[i],
                          use_cache=True, cache_offset=cache_offset)
            new_cache.append(kv)
